...
class YzyController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($data['status'] == 'PAID') {
            ...
            Log::info('【有赞云】付款成功,qr_id=' . $msg['qr_info']['qr_id']);
            $payment = Payment::query()->where('qr_id', $qr_id)->first();
            if (!$payment) {
                Log::error('【有赞云】回调订单不存在,qr_id=' . $qr_id);
                exit();
            }
            //FIXME: 这一步检查貌似没必要exit
            if ($payment->status != '0') {
                Log::warning('【有赞云】回调订单状原状态不是[等待支付]');
                exit();
            }
            ...
            try {
                // 更新支付单
                $payment->pay_way =  $msg['full_order_info']['source_info']['source']['platform'] == 'wx' ? 1 : 2; // 1-微信、2-支付宝
                $payment->status = 1;
                $payment->save();
